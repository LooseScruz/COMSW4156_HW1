...
function createSOR(name, vertices, angle_deltas) {
    ...
    for (var a = 0; a > -360; a -= angle_deltas) {
        for (var i = 0; i < vertices.length; i++) {
            //Rotate vector
            var rotv = rotate_vector3_y(vertices.get(i), a);
...
function SORtoDrawObject(sor) {
    var sorObject = new DrawObject();
    sorObject.name = currentSORname;
    currentSORname = "";
...
function button_save() {
    if (!isDone) {
        ...
        if (currentlySelectedDrawObject != null) {
            saveFile(new SOR(currentlySelectedDrawObject.name,
                currentlySelectedDrawObject.vertices_smooth,
                currentlySelectedDrawObject.indices_smooth));
        }
        else {
...
function load_SOR() {
    ...
    if (typeof fileOut != 'undefined') {
        ...
        var loadObj = SORtoDrawObject(fileOut);
        loadObj.name = fileOut.objName;
        console.log("Loaded SOR: " + loadObj.name);
        drawObjects.push(loadObj);
        draw();
        isDone = true;
    }
}
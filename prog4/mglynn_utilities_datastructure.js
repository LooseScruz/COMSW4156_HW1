...
function Float32ArrayList2 () {
    ...
    this.Push = function (elements)
    {
        ...
        {
            this._data = this.resize(this._data, 2);
        }

        this._data[index] = elements[0];
        this._data[index+1] = elements[1];
    ...
    {
        var index = i * 2;
        return [this._data[index], this._data[index+1]];
    };

    this.set = function(i, vector2)
    {
        var index = i * 2;
        this._data[index] = vector2[0];
        this._data[index+1] = vector2[1];
    };

    this.resize = function(arr, scale)
    {
        var a = new Float32Array(arr.length * scale);
        var len = (arr.length < a.length) ? arr.length : a.length;
        for(var i = 0; i < len; i++)
        {
            a[i] = arr[i];
        }
...
function DrawObject() {
    ...
    this.texture_flat = new Float32ArrayList2();
    ...
    this.texture_smooth = new Float32ArrayList2();
    ...
    this.GetTexture = function () {
        return this.shadingMethod == ShadingMethod.FLAT ? this.texture_flat : this.texture_smooth;
    ...
    this.initializeFlatShading = function (vertices, indices) {
        ...
        for (var i = 0; i < indices.length; i++) {
            ...
            if(i % 2 == 0)
            {
                this.texture_flat.Push([0,0]);
                this.texture_flat.Push([0,1]);
                this.texture_flat.Push([1,0]);
                this.texture_flat.Push([1,0]);
                this.texture_flat.Push([0,1]);
                this.texture_flat.Push([1,1]);
            }
        }
    };

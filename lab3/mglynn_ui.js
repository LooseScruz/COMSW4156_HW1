...
function click(ev, canvas) {
    ...
    if (isDone) {
        ...
        process_click(ColorAtPoint(x_in_canvas, y_in_canvas));
        return;
...
function process_click(color) {
    if (color[0] == diffuse_color.elements[0] * 255
        && color[1] == diffuse_color.elements[1] * 255
        && color[2] == diffuse_color.elements[2] * 255) {
        lightEnabled = (lightEnabled + 1) % 2;
        drawLineStrips[0].active = (drawLineStrips[0].active + 1) % 2;
        draw();
    }
    else if (color[0] == point_light_color.elements[0] * 255
        && color[1] == point_light_color.elements[1] * 255
        && color[2] == point_light_color.elements[2] * 255) {
        pointLightEnabled = (pointLightEnabled + 1) % 2;
        drawObjects[0].active = (drawObjects[0].active + 1) % 2;
        draw();
    }
}